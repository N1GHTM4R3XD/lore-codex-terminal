...
import { VaultState, NamedWhiteboard, Whiteboard, StickerPack } from "@/lib/vault-types";
...
export const NamedWhiteboardsTab = ({ state, update, stickerPacks, onAddStickerPack, onRemoveStickerPack }: Props) => {
  ...
  return (
    <section className="space-y-4">
      ...
      <div className="flex items-stretch gap-1 overflow-x-auto border-b border-border">
        {boards.map((b) => {
          ...
          return (
            ...
            >
              {isRenaming ? (
                <form className="flex items-center gap-1" onSubmit={(e) => { e.preventDefault(); commitRename(); }} onClick={(e) => e.stopPropagation()}>
                  ...
                  <button type="submit" className="text-[hsl(var(--rune))] hover:opacity-80"><Check className="h-3.5 w-3.5" /></button>
                  <button type="button" onClick={() => setRenamingId(null)} className="text-muted-foreground hover:text-foreground"><X className="h-3.5 w-3.5" /></button>
                </form>
              ) : (
                <>
                  <span className="truncate max-w-[120px]">{b.name}</span>
                  <span className={cn("flex items-center gap-0.5 transition-opacity", isActive ? "opacity-60" : "opacity-0 group-hover:opacity-60")} onClick={(e) => e.stopPropagation()}>
                    <button title="Zmień nazwę" onClick={() => startRename(b)} className="hover:text-[hsl(var(--rune))] p-0.5"><Pencil className="h-3 w-3" /></button>
                    {boards.length > 1 && (
                      <button title="Usuń tablicę" onClick={() => deleteBoard(b.id)} className="hover:text-destructive p-0.5"><Trash2 className="h-3 w-3" /></button>
                    )}
                  </span>
                </>
              )}
        ...
        <button onClick={addBoard} title="Nowa tablica"
      ...
      <WhiteboardCanvas
        ...
        onAddStickerPack={onAddStickerPack}
        onRemoveStickerPack={onRemoveStickerPack}
      />
    </section>
  );
};
